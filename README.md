...
| FinalBurn Neo | Neo Geo / Arcade (CPS1/2/3) | 1 | 3 | 0 | Run Ahead 2 single-instance; `rewind_enable = "false"` drift-guard, equals global ([#16374](https://github.com/libretro/RetroArch/issues/16374) closed) |
...
| Mupen64Plus-Next | Nintendo 64 | 2 | 8 | 9 | tvOS software stack: angrylion sw-RDP + cxd4 RSP, no JIT. ParaLLEl-RDP/RSP (need `vulkan`) and GLideN64 (needs GL) compile in but stay inactive under the companion's `video_driver = "metal"`. `mupen64plus-angrylion-multithread = "2"` (worker-thread count, not affinity); `mupen64plus-FrameDuping = "True"`; `mupen64plus-pak1`–`pak4 = "rumble"`. Frontend pins per [Configuration](#configuration) |
...
> [!WARNING]
> Mixing frontend keys and core options in one file causes silent
> failures — RetroArch ignores core option keys in `.cfg` and vice versa.

`.cfg` headers carry a version stamp; `.opt` headers do not — core
...
`rewind_enable`; Mupen `video_threaded`, `audio_sync`, `audio_latency`,
`run_ahead_enabled`, `run_ahead_secondary_instance`, `rewind_enable`),
so a future change to the global `retroarch.cfg` cannot silently move a
pinned core.

Inherited from global, not set per-core: `preemptive_frames_enable`,
`audio_resampler_quality`, `run_ahead_hide_warnings`,
`run_ahead_frames`. No `.cfg` sets a shader preset; assign one per-core
via Quick Menu → Shaders → Manage Presets → Save Core Preset —
recommended presets in
...
- [retroarch-appletv4k](https://github.com/ryanmusante/retroarch-appletv4k) — global `retroarch.cfg` (74 keys) and the Apple TV 4K setup guide; the per-core files here override it
