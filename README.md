...
| Genesis Plus GX | Genesis / Mega Drive / Sega CD / Master System | 1 (Flawless) | 6 | 6 | MAME YM2612; **v3.1: `genesis_plus_gx_audio_filter` low-pass → off** (inert `genesis_plus_gx_lowpass_range = "55"` key removed; authentic 55 Hz MD roll-off removed for CPU headroom; restore per-game if preferred); integer overscale 224p @ 4K; per-game BRAM isolation for Sega CD; Run Ahead per-core |
...
| mGBA | GB / GBC / GBA | 1 (Flawless) | 6 | 3 | Integer overscale GBA 240×160 @ 4K; **v3.1: `mgba_interframe_blending` mix_smart → disabled** (per-frame GPU blend cost removed; flicker returns on alternate-frame titles — Zelda Minish Cap, F-Zero GP Legend, MK Super Circuit — restore mix_smart per-game if preferred); **v3.1: `mgba_audio_low_pass_filter` enabled → disabled** (IIR filter cost removed; brighter/harsher GBA audio); Run Ahead per-core. Handheld LCDs — apply `handheld/lcd-grid-v2.slangp` via Quick Menu → Shaders → Save Core Preset for LCD aesthetic (see companion §8) |
| Snes9x | SNES | 1 (Flawless) | 6 | 1 | Integer overscale 224p @ 4K; Run Ahead per-core |
| Mupen64Plus-Next | Nintendo 64 | 2 (Good) | 5 | 6 | No JIT; cached interpreter; Angrylion sw RDP + CXD4; native 320×240; `mupen64plus-angrylion-multithread = "all threads"` (upstream default; maximizes software-rasterizer parallelism across all available cores — frame completion is gated by slowest RDP worker, so any idle core is wasted parallelism; dedicated-function console has no foreground competition for scheduler contention); per-core `.cfg` pins: `video_threaded = "false"` ([#14978](https://github.com/libretro/RetroArch/issues/14978)), `video_frame_delay_auto = "false"` ([#14201](https://github.com/libretro/RetroArch/issues/14201)), `rewind_enable = "false"` ([#18300](https://github.com/libretro/RetroArch/issues/18300)), `run_ahead_enabled = "false"` (HW-GL state thrash on serialize/unserialize). Mupen inherits global low-latency posture (`audio_latency = 48`, `video_max_swapchain_images = 2`) — no per-core stutter-mitigation tax |
...
| `audio_latency` | `48` | PCSX-ReARMed pin (drift-guard on global); all other cores inherit global `48` (minimum viable buffer on tvOS coreaudio) |
...
Global `retroarch.cfg` sets the shader pipeline gate (`video_shader_enable = "false"` as of companion v3.1; **shaders disabled by default for max GPU headroom at 4K60**) and retains the global preset path (`video_shader = "../shaders/shaders_slang/crt/crt-easymode.slangp"`) as inert fallback for users who flip the gate back on. None of the 8 per-core `.cfg` files set `video_shader` — every core inherits whatever the global specifies. When the gate is on, crt-easymode applies to all cores (selected as default because it is integer-scale safe with no shader geometry and produces a cleaner 4K phosphor mask than crt-aperture, which it replaced in companion v2.88). Per-game or per-core shader overrides use RetroArch's Save Core Preset / Save Game Preset UI (see `retroarch-appletv4k/README.md` §8), which writes dedicated `.slangp` files rather than `.cfg` entries. mGBA users who prefer the handheld-LCD aesthetic should apply `../shaders/shaders_slang/handheld/lcd-grid-v2.slangp` via Save Core Preset.
